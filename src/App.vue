...
export default {
  ...
  methods: {
    ...
    // Debounce chart rendering to prevent glitching
...
renderChart() {
    ...
    this.destroyChart(); // Ensure no old instance is lingering
    ...
    if (this.currentMode === 'running') {
        ...
        const filteredData = this.fetchedData.filter(d => d && d.date);
        if (filteredData.length === 0) {
            this.error = "No swimming data available";
            return;
        }
        labels = filteredData.map(d => {
            const date = new Date(d.date);
    ...
    // Chart creation within try-catch block
    try {
